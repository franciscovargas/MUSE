...
def main():
    ...
    pd.DataFrame(X, index=dico_keys).to_csv("data/x_vecs.csv", header=[T,D] + [None] * (D-2), sep=" ")
    pd.DataFrame(Y, index=dico_keys).to_csv("data/z_vecs.csv", header=[T,D] + [None] * (D-2), sep=" ")
    pd.DataFrame(Y, index=dico_keys).to_csv("data/y_vecs.csv", header=[T,D] + [None] * (D-2), sep=" ")
    pd.DataFrame(W, index=dico_keys).to_csv("data/w_vecs.csv", header=[T,D] + [None] * (D-2), sep=" ")
