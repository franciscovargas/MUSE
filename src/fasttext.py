...
class FastVector:
    ...
    def subset(self, words):
        """
        For test set creation. Creates a new FastVector instance from the current
        one with a smaller subset of words specified by :params words:

        :params words[list[str]]: list of words to create subset dataset with

        :returns [FastVector]: returns FastVector with subset specified by words
        """
        obj = deepcopy(self)
        obj.n_words = len(words)
        obj.embed = np.zeros((obj.n_words , self.n_dim))
        obj.word2id = {}
        obj.id2word = []
        for i, word in enumerate(words):
            obj.word2id[word] = i
            obj.embed[i] = self.embed[self.word2id[word]]
            obj.id2word.append(word)

        return obj

    def set_embeddings(self, Z):
        if Z.shape[0] != self.embed.shape[0]:
            raise AttributeError("Different number of embeddings")
        self.embed = Z
    ...
    def translate_inverted_softmax(self, source_vector, source_space, nsamples,
                                   beta=0.10, batch_size=100, recalculate=True):
        """
    ...
    def dot(self, other):
        """
        dots embedding matrices of self and other
        :param self[fasttext]:
        :param other[fasttext]:
        """
        return self.embed.dot(other.embed)
